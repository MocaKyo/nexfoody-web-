...
export default function Carrinho() {
  ...
  const {
    ...
    finalizarPedido, removeFromExtras, updateExtraQty, pontos,
  } = useStore();
...
try {
  ...
  return (
    <div className="page" style={{ paddingBottom: 160 }}>
      ...
      <div style={{ position: "fixed", bottom: 60, left: 0, right: 0, padding: "12px 16px", background: "rgba(10,4,20,0.95)", backdropFilter: "blur(10px)", borderTop: "1px solid var(--border)", zIndex: 50 }}>
        <button onClick={handleFinalizar} disabled={loading || items.length === 0} style={{
          width: "100%", padding: "16px",
          background: items.length === 0 ? "var(--bg3)" : "linear-gradient(135deg, var(--purple2), var(--purple))",
          border: "none", borderRadius: 14,
          color: items.length === 0 ? "var(--text3)" : "#fff",
          fontFamily: "'Outfit', sans-serif", fontWeight: 800, fontSize: "1rem",
          cursor: items.length === 0 ? "not-allowed" : "pointer",
          display: "flex", alignItems: "center", justifyContent: "space-between",
          boxShadow: "0 8px 24px rgba(90,45,145,0.4)",
        }}>
          <span>{loading ? "⏳ Processando..." : pagamento === "pix" ? "📱 Confirmar e gerar PIX" : pagamento === "cartao_online" ? "💳 Pagar com cartão" : "💬 Enviar pedido"}</span>
