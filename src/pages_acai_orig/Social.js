          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 2, padding: "0 2px" }}>
            {posts.map(p => (
              <div key={p.id} style={{ paddingTop: "100%", position: "relative", background: "var(--bg3)" }}>
                {p.foto && <img src={p.foto} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />}
                {!p.foto && <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.5rem" }}>🫐</div>}
              </div>
            ))}
          </div>
        )}
...
function TabMural({ user, userData }) {
  ...
  const publicar = async () => {
    if (!novaFoto && !novaLegenda.trim()) { toast("Adicione uma foto ou legenda.", "error"); return; }
    setPostando(true);
    try {
      let fotoUrl = null;
      ...
      setNovaFoto(null); setNovaLegenda(""); setNovaProduto(""); setPreview(null);
  ...
  return (
    <div>
      ...
      <div style={{ background: "var(--bg2)", border: "1px solid var(--border)", borderRadius: 16, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <label style={{ flex: 1, padding: "8px", textAlign: "center", background: "rgba(245,197,24,0.08)", border: "1px dashed rgba(245,197,24,0.3)", borderRadius: 10, cursor: "pointer", fontSize: "0.78rem", color: "var(--gold)", fontFamily: "'Outfit',sans-serif" }}>
            📷 {novaFoto ? "Foto selecionada ✓" : "Adicionar foto"}
            <input type="file" accept="image/*" style={{ display: "none" }} onChange={handleFoto} />
          </label>
          <button onClick={publicar} disabled={postando} style={{ padding: "8px 20px", background: "linear-gradient(135deg, var(--purple2), var(--purple))", border: "none", borderRadius: 10, cursor: "pointer", color: "#fff", fontFamily: "'Outfit',sans-serif", fontWeight: 700, fontSize: "0.85rem" }}>
            {postando ? "⏳" : "Publicar"}
          </button>
        </div>
      </div>
