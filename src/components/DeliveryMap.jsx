...
export default function DeliveryMap({ clientePos, setClientePos, raioKm }) {
  ...
  const bairrosNaoAtendidos = config.bairrosNaoAtendidos || [];
      ...
      <div style={{
        ...
        {clientePos ? (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div>
                <div style={{ fontSize: "0.82rem", fontWeight: 600, color: dentroRaio ? "var(--green)" : "var(--red)" }}>
                  {dentroRaio
                    ? `Você está dentro do raio de entrega! (${distance?.toFixed(1)} km)`
            ...
            {dentroRaio && bairrosNaoAtendidos.length > 0 && (
              <div style={{ background: "rgba(245,197,24,0.08)", border: "1px solid rgba(245,197,24,0.25)", borderRadius: 8, padding: "8px 10px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
                  <span style={{ fontSize: "0.75rem", color: "var(--gold)" }}>
                    ⚠️ Não entregamos em alguns bairros.
                  </span>
                  <button
                    onClick={() => setMostrarBairros(v => !v)}
                    style={{ background: "none", border: "none", cursor: "pointer", fontSize: "0.72rem", color: "var(--gold)", fontWeight: 700, padding: 0, textDecoration: "underline", fontFamily: "'Outfit', sans-serif" }}
                  >
                    {mostrarBairros ? "Ocultar" : "Confira aqui"}
                  </button>
                </div>
                {mostrarBairros && (
                  <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 5 }}>
                    {bairrosNaoAtendidos.map(b => (
                      <span key={b} style={{ background: "rgba(239,68,68,0.12)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 20, padding: "2px 10px", fontSize: "0.72rem", color: "var(--red)" }}>{b}</span>
                    ))}
                  </div>
                )}
              </div>
