...
import { collection, query, orderBy, limit, onSnapshot, doc, getDoc, setDoc, deleteDoc, serverTimestamp, getCountFromServer } from "firebase/firestore";
...
export default function FeedLoja() {
  ...
  // Verifica se já segue e conta seguidores
  useEffect(() => {
    const contarSeguidores = async () => {
      try {
        const snap = await getCountFromServer(collection(db, "seguidoresLoja"));
        setTotalSeguidores(snap.data().count);
      } catch {
        // fallback silencioso
      }
    };
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontWeight: 700, fontSize: "1rem", color: "var(--text1)", marginBottom: 2 }}>
              {nome}
            </div>
              ...
              <div key={post.id} style={{
                ...
                <div style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                    ...
                    <span style={{ fontSize: "0.68rem", color: "var(--text3)", marginLeft: "auto" }}>
                      {formatDate(post.createdAt)}
                    </span>
                  </div>
                  <p style={{ fontSize: "0.88rem", color: "var(--text2)", lineHeight: 1.5, margin: 0 }}>
                    {post.texto}
                  </p>
                </div>
              </div>
