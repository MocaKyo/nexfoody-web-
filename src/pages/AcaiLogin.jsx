...
export default function AcaiLogin() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#07030f", fontFamily: "'Outfit', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ width: 56, height: 56, borderRadius: 16, background: "linear-gradient(135deg,#7c4dbd,#5a2d91)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.8rem", boxShadow: "0 8px 32px rgba(124,77,189,.4)" }}>
          🍓
        </div>
      </div>

      <h1 style={{ fontFamily: "'Playfair Display', serif", fontWeight: 900, fontSize: "1.6rem", color: "#fff", marginBottom: 6, textAlign: "center" }}>
        Entrar no <span style={{ color: "#c4b5fd" }}>Açaí Puro Gosto</span>
      </h1>
      <p style={{ fontSize: "0.85rem", color: "rgba(255,255,255,.45)", marginBottom: 32, textAlign: "center" }}>
        Faça login para pedir e acompanhar pedidos
      </p>
      ...
      <form style={{ width: "100%", maxWidth: 340, display: "flex", flexDirection: "column", gap: 14 }} onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          disabled={loading}
          style={{ width: "100%", padding: "15px", background: "linear-gradient(135deg,#7c4dbd,#5a2d91)", border: "none", borderRadius: 14, fontWeight: 800, fontSize: "1rem", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontFamily: "'Outfit', sans-serif" }}
        >
          {loading ? (
            <div style={{ width: 20, height: 20, border: "2px solid rgba(255,255,255,.3)", borderTop: "2px solid #fff", borderRadius: "50%", animation: "spin 1s linear infinite" }} />
          ) : "Entrar"}
        </button>
      ...
      <p style={{ marginTop: 28, fontSize: "0.82rem", color: "rgba(255,255,255,.4)" }}>
        Não tem conta?{" "}
        <Link to="/acai/cadastro" style={{ color: "#c4b5fd", fontWeight: 700, textDecoration: "none" }}>
          Criar conta grátis
        </Link>
      </p>

      <p style={{ marginTop: 16, fontSize: "0.72rem", color: "rgba(255,255,255,.25)" }}>
        <Link to="/acai/welcome" style={{ color: "rgba(255,255,255,.35)", textDecoration: "none" }}>
          ← Voltar
        </Link>
      </p>
    </div>
