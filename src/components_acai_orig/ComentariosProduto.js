...
export default function ComentariosProduto({ produto, onClose }) {
      ...
      if (navigator.share) {
        ...
        if (isMobile) {
          ...
          try {
            ...
            await navigator.share({ title: "Açaí Puro Gosto", text: texto, files: [file] });
          } catch {
            await navigator.share({ title: "Açaí Puro Gosto", url: fotoUrl });
          }
        } else {
  ...
  const compartilharTexto = async (comentario) => {
    try {
      ...
      if (navigator.share) {
        await navigator.share({ title: "Açaí Puro Gosto", text: texto });
      } else {
...
`}</style>
        ...
        <div style={{ padding: "6px 16px 4px", display: "flex", flexDirection: "column", gap: 6, flexShrink: 0 }}>
          ...
          {/* Top 3 do ranking + Cupom */}
          {(topUsers.length > 0 || temCupons) && (
            <div style={{ display: "flex", gap: 6, overflow: "hidden" }}>
              ...
              {topUsers.length > 0 ? (
                <div style={{ flex: 3, display: "flex", gap: 6 }}>
                  {topUsers.slice(0, 3).map(u => (
                    <div
                      key={u.id}
                      onClick={() => { onClose(); navigate(`/perfil/${u.id}`); }}
                      style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, padding: "3px 2px", cursor: "pointer" }}
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                        <div style={{ width: 22, height: 22, borderRadius: "50%", background: "var(--surface)", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.6rem", border: `1.5px solid ${u.posicao === 1 ? "#f5c518" : u.posicao === 2 ? "#c0c0c0" : "#cd7f32"}` }}>
                          {u.photoURL ? <img src={u.photoURL} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : (u.displayName || u.nome || "?")?.[0]?.toUpperCase()}
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 0 }}>
                          <span style={{ fontSize: "0.55rem", lineHeight: 1 }}>{getBadges(u.posicao, u.rankingPts || 0)[0]?.emoji} <span style={{ color: "var(--gold)", fontWeight: 700, fontSize: "0.5rem" }}>{Math.floor(u.rankingPts || 0)}</span></span>
                          <span style={{ fontSize: "0.45rem", color: "var(--text3)", lineHeight: 1 }}>pts</span>
                        </div>
                      </div>
                      <div style={{ fontSize: "0.48rem", fontWeight: 700, color: "var(--gold)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: "100%", textAlign: "center", marginTop: 1 }}>
              ...
              {temCupons && (
                <button
                  ...
                  style={{ flex: 1, background: "linear-gradient(135deg, #16a34a, #15803d)", border: "none", borderRadius: 8, padding: "3px 4px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 2px 8px rgba(22,163,74,0.35)", minHeight: 26 }}
                >
                  <span key={fraseAtual} style={{ fontSize: "0.62rem", fontWeight: 700, color: "#fff", textAlign: "center", animation: "fadeIn 0.3s ease" }}>{fraseAtual}</span>
                </button>
              )}
            </div>
          )}
